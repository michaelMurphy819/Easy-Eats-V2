...
export default function CollectionPage() {
  ...
  const fetchSavedRecipes = useCallback(async (pageNum: number, isInitial: boolean = false) => {
    ...
    if (!error && data) {
      setSavedRecipes(prev => isInitial ? data : [...prev, ...data]);
  ...
  useEffect(() => {
    if (inView && hasMore && !loading && !isFetchingMore) {
      ...
      setPage(nextPage);
      fetchSavedRecipes(nextPage);
    }
  }, [inView, hasMore, loading, isFetchingMore, page, fetchSavedRecipes]);

  // Realtime listener resets to page 0 to show newest saves
  useEffect(() => {
        ...
        {savedRecipes.length > 0 ? (
          <>
            ...
            <div ref={ref} className="h-10 w-full flex justify-center items-center">
                {isFetchingMore && <div className="w-5 h-5 border-2 border-primary/30 border-t-primary rounded-full animate-spin" />}
            </div>
          </>
        ) : (
