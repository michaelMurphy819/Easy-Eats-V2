...
import { Settings, Grid, Bookmark, Tag, ChefHat, Leaf, MilkOff, UserCircle } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-16 mb-10">
          ...
          <div className="relative w-32 h-32 rounded-full p-[3px] bg-gradient-to-tr from-primary to-primary/20">
            <div className="w-full h-full rounded-full bg-background flex items-center justify-center text-5xl border-4 border-background overflow-hidden shadow-sm">
              {user?.avatar_url ? (
              ...
              <div className="flex items-center gap-2 justify-center md:justify-start pt-1">
                {isOwner ? (
                  <>
                    <Link 
                      href="/profile/edit" 
                      className="flex items-center gap-2 px-4 py-2 bg-foreground/[0.05] hover:bg-foreground/[0.1] text-foreground rounded-xl text-sm font-bold transition-colors border border-border"
                    >
                      <UserCircle size={18} className="text-foreground/60" />
                      Edit profile
                    </Link>
                    <Link 
                      href="/settings" 
                      className="p-2 text-foreground/40 hover:text-foreground hover:bg-foreground/[0.05] rounded-lg transition-all"
                      title="App Settings"
                    >
                      <Settings size={22} />
                    </Link>
        ...
        <div className="flex justify-center gap-12">
          <button 
            ...
            className={`flex items-center gap-2 pt-4 pb-4 text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'posts' ? 'border-t-2 border-foreground -mt-[2px] text-foreground' : 'text-foreground/30 hover:text-foreground/60'}`}
          >
            <Grid size={16} /> Posts
          </button>
          {isOwner && (
            <button 
              ...
              className={`flex items-center gap-2 pt-4 pb-4 text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'saved' ? 'border-t-2 border-foreground -mt-[2px] text-foreground' : 'text-foreground/30 hover:text-foreground/60'}`}
            >
          ...
          <button 
            ...
            className={`flex items-center gap-2 pt-4 pb-4 text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'tagged' ? 'border-t-2 border-foreground -mt-[2px] text-foreground' : 'text-foreground/30 hover:text-foreground/60'}`}
          >
            <Tag size={16} /> Tagged
          </button>
        </div>
